...
export function AnimatedToastStack({
  ...
  useEffect(() => {
    const target = shouldPortal ? (portalRoot ?? document.body) : null;
    queueMicrotask(() => setPortalTarget(target));
          ...
          <div className="flex items-start gap-3">
            ...
            <div className={cn("min-w-0 flex-1", classNames?.content)}>
              ...
              {toast.action ? (
                <button
                  ...
                  className={cn(
                    "mt-2 inline-flex h-7 items-center rounded-full bg-primary/[0.06] px-3 text-xs font-medium text-foreground transition-colors hover:bg-primary/[0.1] cursor-pointer",
                    classNames?.action,
                  )}
                >
                  {toast.action.label}
            ...
            {canDismiss ? (
              <button
                ...
                className={cn(
                  "inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-primary/[0.06] hover:text-foreground cursor-pointer",
                  classNames?.close,
                )}
              >
