    ...
    <div className="flex flex-col">
      <div className="flex flex-row items-start">
        <CalendarDropdown
          ...
          locale={locale}
        />
        <div ref={scrollRef} className="w-[130px] pr-2 py-3 h-[320px]">
            ...
            <div className="flex flex-col gap-1 pr-2">
              {timeSlots.map((timeValue) => {
                ...
                return (
                  <Button
                    key={timeValue}
                    className="w-full text-center justify-center px-2 text-xs h-7 font-mono"
...
>(function DateTimePickerRange(
  ...
  // Sync controlled value if prop changes
  ...
  const displayText = hasSelection ? (
    <span className="flex items-center gap-1.5 truncate">
      {fromDate ? (
        <span>
          {format(fromDate, "MMM d, yyyy", { locale })} {fromTime}
        </span>
      ) : (
      ...
      {toDate ? (
        <span>
          {format(toDate, "MMM d, yyyy", { locale })} {toTime}
        </span>
      ) : (
  ...
  const pickerContent = (
    <div className="flex flex-col bg-surface rounded-lg">
      ...
      {/* Optional Apply button footer */}
      {applyLabel && (
        <div className="flex items-center justify-end gap-2 border-t border-border p-2.5 bg-surface-subtle/50 rounded-b-lg">
          <Button
            type="button"
            variant="outline"
            size="sm"
  ...
  return (
    <>
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        <PopoverContent
          className="w-auto p-0 border border-border shadow-lg"
          align="end"
        >
          {pickerContent}
        </PopoverContent>
