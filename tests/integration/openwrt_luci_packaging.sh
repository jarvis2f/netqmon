...
assert_contains "$UPDATE_HELPER" "apk add --allow-untrusted"
assert_contains "$UPDATE_HELPER" "package_format"
assert_contains "$UPDATE_HELPER" "github.com/jarvis2f/netqmon/releases/download/.*/netqmon-agent_.*\\.ipk"
assert_contains "$UPDATE_HELPER" "github.com/jarvis2f/netqmon/releases/download/.*/netqmon-agent-.*\\.apk"
...
assert_contains "$MANIFEST_GENERATOR" "package_format"
...
assert_contains "$INSTALLER" 'apk add --allow-untrusted "\$agent_pkg" "\$luci_pkg"'
